...
\\
ID3 can use two different measures to decide which is the best attribute, 
information gain or the Gini Impurity.
\\
In order to compute the Information Gain we have to first define entropy. 
Infomration Entropy is a common way to measure the impurity in the data set, it 
is calculated as shown in Equation \ref{eq:entropy}
\begin{equation}
 E(S) = \sum_{i=1}^C -p(i) \ log_2(p(i))
 \label{eq:entropy}
\end{equation}
\begin{equation}
 Gain(S, A) = E(S) - \sum_{i}^{C} \frac{|S_v|}{S} E(S_v)
 \label{eq:gain}
\end{equation}
\\
Gini Impurity
